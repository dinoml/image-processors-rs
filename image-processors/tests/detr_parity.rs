...
use image_processors::{
    DetrImageProcessor, DetrImageProcessorConfig, ImageFrame, ImageSize, Layout, PixelFormat,
    ResizeParity, Tensor, TensorData,
};
use serde::Deserialize;

const FIXTURE: &str = include_str!("fixtures/transformers_detr.json");
const FIXTURE_SCHEMA: &str = "image-processors.transformers-parity.v1";
const PIXEL_VALUES: &str = "pixel_values";
const PIXEL_MASK: &str = "pixel_mask";
...
#[test]
fn detr_fixture_schema_matches_expected_version() {
...
#[test]
fn detr_output_matches_transformers_fixture() {
    for case in fixture_cases() {
        ...
        let processor = DetrImageProcessor::new(config).expect("fixture DETR config should build");

        let output = processor
            .preprocess_image_output(&frame)
            .expect("DETR preprocessing should succeed");
        ...
        assert_pixel_mask_matches(
            &case,
            output.pixel_mask().expect("missing pixel_mask tensor"),
        );
        assert_eq!(
...
fn fixture_cases() -> Vec<ParityCase> {
    let cases = load_fixture()
        .cases
        .into_iter()
        .filter(|case| case.family == "detr")
        .collect::<Vec<_>>();
    assert_eq!(cases.len(), 2, "fixture should cover both DETR backends");
...
fn assert_pixel_mask_matches(case: &ParityCase, tensor: &Tensor) {
    let reference = case
        .outputs
        .get(PIXEL_MASK)
        .expect("fixture missing pixel_mask");
    assert_eq!(reference.dtype, "int64");
    assert_eq!(reference.shape.len(), 3);
    assert_eq!(
        tensor.shape(),
        [
            reference.shape[0],
    ...
    assert_eq!(tensor.layout(), Layout::NCHW, "{}", case.model_id);
    assert!(
        matches!(tensor.data(), TensorData::Bool(_)),
        "{} produced {:?}, expected bool mask",
        case.model_id,
        tensor.data().dtype()
    );

    let expected: Vec<bool> = reference_i64(reference)
        .into_iter()
        .map(|value| value != 0)
        .collect();
    let actual = tensor.data().to_vec::<bool>();
    assert_eq!(actual, expected, "{} pixel_mask", case.model_id);
}
...
fn assert_float_values_close(case: &ParityCase, actual: &[f32], expected: &[f32], shape: &[usize]) {
    ...
    assert!(
        ...
        "DETR pixel_values drift too high: {}",
        diff.describe(shape)
    );
}
...
fn assert_close(actual: f64, expected: f64, tolerance: f64, label: &str) {
    ...
    assert!(
        delta <= tolerance,
        "DETR {label} differs from Transformers fixture: actual={actual:.8}, expected={expected:.8}, delta={delta:.8}, tolerance={tolerance:.8}"
    );
}
